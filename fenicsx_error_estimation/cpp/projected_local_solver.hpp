...
void projected_local_solver(fem::Function<T>& eta_h, const fem::Form<T>& a,
...
{
  std::cout << "Projected local solver" << std::endl;
  ...
  // xt::xtensor<T, 1> b_macro = xt::zeros<double>({2 *
  // element_space_dimension}); xt::xtensor<T, 2> Ae_0; xt::xtensor<T, 1> be_0,
  // xe_0, xe;
  ...
  //         // Exterior facet term
  //         L_kernel_exterior_facet(be.data(), L_coeff_cell,
  //         L_constants.data(),
  ...
  //                     xt::view(coordinate_dofs_macro, 0, i,
  //                     xt::all()).begin());
  //       }
  ...
  //                     xt::view(coordinate_dofs_macro, 1, i,
  //                     xt::all()).begin());
  //       }
  ...
  //                     std::next(L_coeff_array_macro.begin(), 2 *
  //                     L_offsets[i]));
  ...
  //             = element_dof_layout.entity_closure_dofs(tdim - 1,
  //             local_facet);
  ...
  //   // Compute indicator
  //   etae[0] = 0.0;
  //   L_eta_kernel_domain_integral(etae.data(), xe.data(),
  //   L_eta_constants.data(),
  //                                coordinate_dofs.data(), nullptr, nullptr);
